...
# Author: Ousmane Diallo (improved version)
# Date: 2026-02-03
# Version: 2.1 (Cleaned and consolidated)
#
# Description:
...
# Load controlled terminology with error handling
study_ct <- tryCatch(
  {
    read.csv(
...
ct_ds <- study_ct %>% filter(codelist_code == "C66727")
...
ds_dsdecod <- withCallingHandlers(
  ...
  dplyr::select(dplyr::all_of(idv), DSDECOD)

# --- Merge mapped DSDECOD back to the full ds_raw (preserve all rows) ---
# Rows not sent to assign_ct() (e.g., randomized, final lab visit) keep DSDECOD = NA.
ds_raw <- ds_raw %>%
  dplyr::left_join(ds_dsdecod, by = idv)
...
ds_dscat <- ds_raw %>%
  mutate(
    DSCAT = case_when(
      ...
      !is.na(IT.DSDECOD) & IT.DSDECOD == "Randomized" ~
        "PROTOCOL MILESTONE",
      
      # Priority 2: Other event for verbatim text
      !is.na(OTHERSP) & nchar(OTHERSP) > 0 ~
        "OTHER EVENT",
...
ds_dtc <- ds_raw %>%
  mutate(
    ...
    date_parsed = case_when(
      grepl("^\\d{2}-\\d{2}-\\d{4}$", DSDTCOL) ~
        as.Date(DSDTCOL, format = "%m-%d-%Y"),
      TRUE ~ as.Date(NA)
    ),
    ...
    DSDTC = case_when(
      !is.na(date_parsed) & !is.na(time_parsed) ~
        paste0(format(date_parsed, "%Y-%m-%d"), "T", time_parsed),
      !is.na(date_parsed) ~
        format(date_parsed, "%Y-%m-%d"),
      !is.na(time_parsed) ~
        paste0("T", time_parsed),
      TRUE ~
        NA_character_
    )
  ) %>%
